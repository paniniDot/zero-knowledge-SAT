...
\begin{definizione}[Bounded NIZK proof system] 
    Siano $(P, V)$ una coppia sender-receiver. Diremo che $(P, V)$ dove $P(\cdot,\cdot)$ è un algoritmo \emph{random selector}\footnote{Spiegalo} a due input e $V(\cdot, \cdot, \cdot)$ un algoritmo polinomiale. Diremo che $(P, V)$ è un \emph{bounded NIZK proof system} per un linguaggio $L_n$ se esiste una costante positiva $c$ tale che
    \begin{enumerate}
        \item \emph{Completezza}. Per ogni $x \in L_n$ e per ogni $n$ adeguatamente grande (cos'è $n$?) vale
        \begin{equation*}
        ...
        \end{equation*}
        \item \emph{Soundness}. Per ogni $x \notin L_n$, dato un prover impostore $\hat{P}$ e per ogni $n$ adeguatamente grande vale
        \begin{equation*}
            Pr(\sigma \overset{R}{\leftarrow} \left\{ 0, 1\right\}^{n^c}; Proof \overset{R}{\leftarrow} \hat{P}(\sigma, x) : V(\sigma, x, Proof) = 1) < \frac{1}{3}
        \end{equation*}
        \item \emph{Zero-knowledge}. Per ogni $x \in L_n$, dato un prover onesto $P$ e per ogni $n$ adeguatamente grande vale
    \end{enumerate}
\end{definizione}
